...
const { canModifyQueue, STAY_TIME, LOCALE } = require("../util/DevbotUtil");
const i18n = require("i18n");
i18n.setLocale(LOCALE);

module.exports = {
  async play(song, message) {
    const { SOUNDCLOUD_CLIENT_ID } = require("../util/DevbotUtil");
    ...
    const dispatcher = queue.connection
      ...
      .on("finish", () => {
        ...
        if (queue.loop) {
          // se o loop estiver ativado, empurre a música de volta para o final da fila
          // para que possa se repetir indefinidamente
          let lastSong = queue.songs.shift();
          queue.songs.push(lastSong);
          module.exports.play(queue.songs[0], message);
        } else {
          // Tocar recursivamente a próxima música
          queue.songs.shift();
          module.exports.play(queue.songs[0], message);
        }
      })
